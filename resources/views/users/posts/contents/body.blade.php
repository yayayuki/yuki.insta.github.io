...
<div class="card-body">
    ...
    <p class="text-uppercase text-secondary xsmall">
        ...
        {{-- {{ $post->created_at->diffForHumans(); }} --- '28Hours ago" this is usefull if you use jQuery and Javascript--}}
    </p>

    {{-- 
        date(format, unix time) --- translate a unix time to a specific format
        strtotime(timestamp) --- string to time, translate a timestamp to unix time, timestamp is coming from the database

        Examples:
        (step 1) date('D, M d Y', strtotime('2024-07-08 10:59:56'))
        (step 2) date('D, M d Y', 1720436396) <- unix time (seconds since JAN 01 1970)
